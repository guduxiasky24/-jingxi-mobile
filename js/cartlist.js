	...
	var b = function() {
		...
		if (h.length > 0) {
			h.children("a").click(function() {
				location.href = "/mycart/payment.html?forward=" + forward
			})
		}
			...
			if (r > 0) {
				...
				g
						...
						.html(
								"\u603b\u5171"
										+ $shortName
										+ '<span class="orange arial z-user">'
										+ r
										+ '</span>\u4e2a\u5546\u54c1  \u5408\u8ba1\u91d1\u989d\uff1a<span class="orange arial">'
										+ q + ".00</span> \u5143")
			} else {
				g.remove()
			}
		...
		var d = function() {
			var z = $(this);
			var t = z.attr("id");
			var v = t.replace("txtNum", "");//商品期号
			var q = z.next().next();//先购买个数
			var r = parseInt(z.next().next().next().val());//剩余个数
			var sp = parseInt(z.next().next().next().next().val());//单价
		...
		/*var p = function(r, u) {
			...
			if (r > 100) {
				if (q.length == 0) {
					var s = $('<div class="z-Cart-tips">\u5df2\u8d85\u8fc7100\u4eba\u6b21\uff0c'
							+ $shortName
							+ "\u5b58\u5728\u4e00\u5b9a\u98ce\u9669\uff0c\u8bf7\u8c28\u614e\u53c2\u4e0e\uff01</div>");
					t.prepend(s)
				}
		...
		var l = function() {
			var q = $(this);
			var s = q.next().next();//先购买个数
			var r = parseInt(q.next().next().next().val());//剩余个数
			var sp = parseInt(q.next().next().next().next().val());//单价
			var t = Math.ceil(q.val()/sp)*sp;
			if(t <= 0){
				if(sp > r){
					t  = r;
				}else{
		...
		var k = function(w, v) {
			...
			var r = parseInt(v.next().next().next().val());//剩余数
			var q = v.next().next();//购物车当前数
			var sp = parseInt(v.next().next().next().next().val());
			var t = parseInt(q.val()) + w;
			if(w == -1){
				t = Math.ceil((parseInt(q.val()) - sp)/sp)*sp;
			}else{
				t = Math.ceil((parseInt(q.val()) + sp)/sp)*sp;
			}
			if(t<=0){
