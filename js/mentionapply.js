$(function() {
	...
	var a = function() {
		...
		$.fn.formatChnInput = function(C) {
			...
			E.bind("dragenter", function() {
				return false
			}).bind(
					"onpaste",
					function() {
						return !clipboardData.getData("text").match(
								/[^\u4E00-\u9FA5]/g)
					}).bind("focus", function() {
				$(this).css("color", k);
				if (H == null) {
		...
		var l = function() {
			GetJPData("", "getUserBrokerageApplyLastest", "", function(E) {
				if (E.code == 0) {
					...
					i.val(F).css("color", k);
					r.val(
							E.bankAccount.replace(/\s/g, "").replace(
									/(\d{4})(?=\d)/g, "$1 ")).css("color", k);
					u.val(E.telePho).css("color", k)
				} else {
					if (E.code == 2) {
						b("您的佣金余额不足100元");
						setTimeout(function() {
							location.href = "/member/sharelist-"
									+ GetRandomNum(100000, 999999) + ".do"
						}, 2000)
					}
				}
			})
		...
		/*var m = function(F, E, C, D) {
			...
			var B = function(G) {
				if (G.code == 0) {
					...
					for (var H = 0; H < K.length; H++) {
						I += '<option value="' + K[H].areaID + '" zip="'
								+ K[H].areaZip + '" txt="' + K[H].areaName
								+ '">' + K[H].areaName + "</option>"
					}
					E.append(I);
			...
			if (F > -1) {
				GetJPData("", "getChildArea", "areaID=" + F, B)
			}
		};
		m(1, n, "A", l);
		n.change(function() {
			...
			if (B == -1) {
				n.css("color", o);
				p.html('<option value="-1" zip="">' + s + "</option>").css(
						"color", o)
			} else {
				n.css("color", k);
				m(B, p, "B");
						...
						function() {
							if (z()) {
								...
								var F = function(H) {
									...
									var I1='{"money":"'+t+'","userName":"'+h+'","bankName":"'+x+'","subBank":"'+f+'","bankNo":"'+e+'","phone":"'+A+'"}';
									H
											.addClass("grayBtn")
											.html(
									...
									var G = function(J) {
										...
										if (J.state == 1) {
											...
											setTimeout(
													function() {
														location.href = "MentionList.html"
													}, 2000)
										} else {
											if (J.state == -1) {
												b("申请失败,提现金额最低为100元")
									...
									$.ajax({
										...
										/*data:I(),*/
										data:"userJSON="+encodeURIComponent(I1),
										type:"post",
										success:G
									});
									/*GetJPData("", "memberCenterApplyToBank",
											I(), G)*/
								};
								var B = new Date();
								var C = '<div class="record-pop-ups gray6 clearfix"><h4>确认提现信息</h4><ul><li><span>申请时间：</span><p>'
										+ (B.getFullYear() + "-"
												+ (B.getMonth() + 1) + "-" + B
												.getDate())
										...
										+ h
										+ "</p></li><li><span>银行名称：</span><p>"
										+ x
										+ "</p></li><li><span>开户支行：</span><p>"
										+ D
										+ "</p></li><li><span>银行账号：</span><p>"
										+ e
										+ "</p></li><li><span>联系电话：</span><p>"
										+ A
										+ '</p></li></ul><div class="comm-pop-btn"><cite class="return-modify"><a id="closeDiag" href="javascript:;" class="gray6">返回修改</a></cite><cite><a id="submitApply" href="javascript:;" class="orangeBtn">确认提交</a></cite></div></div>';
								$("body").attr("style", "overflow:hidden;");
								$("#loadingPicBlock").append(C).find("#closeDiag")
										.click(function() {
											$("div.record-pop-ups").remove();
											$("div.weixin-mask").hide();
											$("body").attr("style", "");
	...
	Base.getScript($www + "/js/pagedialog.js?v=151104", function() {
		a()
	})
});